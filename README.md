...
- v1.1.2（2026-09-08）: 出走日時の初期値を「端末時刻より先にある最も近い 06:00」に変更。出走日時は**前日の 0:00（JST）より過去に設定できない**（走行中に実際の出走時刻を入れられるよう、前日・当日は許可）。下限より前の保存値は開いた時点で初期値に戻る。出走日時が数日前だと通過時刻が予報の取得範囲（過去 2 日）から外れて「予報範囲外」になる不具合への対処。戻す場合は `git revert` するか、`js/core.js` の `minStart` の下限と `js/app.js` の `enforceStart` / `loadParams` を変える（タグ `v1.1.1` が変更前）
