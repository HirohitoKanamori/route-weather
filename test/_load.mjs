...
export function syntheticSeries(nPoints, fromMs, hours, fn) {
  ...
  for (let p = 0; p < nPoints; p++) {
    const times = [], temp = [], rh = [], mm = [], code = [], cloud = [], ws = [], wd = [];
    for (let h = 0; h < hours; h++) {
      const v = fn(p, h);
      times.push(fromMs + h * 3600e3); temp.push(v.temp); rh.push(v.rh ?? 60); mm.push(v.mm);
      code.push(v.code ?? 1); cloud.push(v.cloud ?? 30); ws.push(v.ws); wd.push(v.wd);
    }
    out.push({ times, temp, rh, mm, code, cloud, ws, wd, validFrom: times[0], validUntil: times[times.length - 1], lat: 0, lon: 0 });
  }
  return out;
}
